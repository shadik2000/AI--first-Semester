...
def plot_color_histograms(image_path: str, sharey: bool = False, bins: int = 255) -> None:
    """
    Plot histograms for each color channel.

    :param image_path: path to image
    :param sharey: whether to use the same y-axis for all three histograms
    :param bins: number of histogram bins
    :return: None
    """
    assert (image_path is not None) and (type(image_path) == str) and (Path(image_path).is_file())
    img = imread(image_path)
    fig, (axr, axg, axb) = plt.subplots(1, 3, sharey=sharey, figsize=(15, 3))
    axr.hist(img[:, :, 0].ravel(), bins=bins, histtype=r'stepfilled', density=True, color=r'red')
    axr.set_xlabel('R Value Range')
    axr.set_ylabel('Normalized Count')
    axg.hist(img[:, :, 1].ravel(), bins=bins, histtype=r'stepfilled', density=True, color=r'green')
    axg.set_xlabel('G Value Range')
    axg.set_ylabel('Normalized Count')
    axb.hist(img[:, :, 2].ravel(), bins=bins, histtype=r'stepfilled', density=True, color=r'blue')
    axb.set_xlabel('B Value Range')
    axb.set_ylabel('Normalized Count')
    fig.tight_layout()
